...
def _saldos_restantes(totals: dict) -> tuple[float, float]:
    ...
    openai_rem  = max(0.0, openai_init - totals.get("openai_usd", 0.0))
...
def _videos_restantes(openai_rem: float, falai_rem: float,
                      avg_openai: float, avg_falai: float) -> str:
    """Devuelve un string legible de cuántos videos quedan por servicio."""
    partes = []
    if avg_openai > 0.001:
        n = int(openai_rem / avg_openai)
        partes.append(f"~{n} con OpenAI")
    if avg_falai > 0.001:
        n = int(falai_rem / avg_falai)
        partes.append(f"~{n} con fal.ai")
    if not partes:
        return "ilimitados (arquitectura de clips)"
    return " / ".join(partes)
...
def mostrar_saldo_inicial():
    ...
    openai_rem, falai_rem = _saldos_restantes(totals)
    avg = _costo_promedio(runs)
    completados = totals.get("videos_completados", 0)

    # Si el avg es casi $0 para openai/falai, arquitectura de clips activa
    ultimas = runs[-3:] if len(runs) >= 3 else runs
    avg_openai = (sum(r.get("openai_usd", 0) for r in ultimas) / len(ultimas)) if ultimas else 0
    avg_falai  = (sum(r.get("falai_usd",  0) for r in ultimas) / len(ultimas)) if ultimas else 0
    est = _videos_restantes(openai_rem, falai_rem, avg_openai, avg_falai)

    log.info(
        f"💰 Saldo estimado: OpenAI ~${openai_rem:.2f} | fal.ai ~${falai_rem:.2f}"
        f" | Videos restantes: {est}"
        f" | Completados: {completados} | Avg costo: ~${avg:.3f}"
    )

    # Alertas de saldo bajo
    if float(os.getenv("OPENAI_INITIAL_BALANCE", 0)) > 0 and openai_rem < _ALERTA_USD:
        log.warning(f"  ⚠️  SALDO BAJO — OpenAI: ${openai_rem:.2f} restante")
...
def registrar_run(run_log: dict):
    ...
    entrada = {
        ...
        "openai_usd":     costos.get("imagenes_usd",  0.0),
    ...
    t = data["totals"]
    for key in ("anthropic_usd", "elevenlabs_usd", "openai_usd", "falai_usd"):
...
def mostrar_resumen_run(run_log: dict):
    ...
    log.info(f"   Imágen  (OpenAI):     ${oa_usd:.3f}{clips_flag}")
    ...
    log.info(f"   OpenAI total:     ${totals.get('openai_usd',     0):.3f}  → saldo restante ~${openai_rem:.2f}")
